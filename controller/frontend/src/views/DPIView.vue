...
onMounted(() => {
  timer = setIntervalImmediate(() => {
    getHostLastHourStatsNetifyStatsByHostLastDayGet().then((res) => {
      if (res.data) {
      ...
      <el-card header="Applications">
        <Doughnut
          :data="{
            ...
            datasets: [
              {
                data: dpi_data ? dpi_data?.categories.map((cat) => cat.download + cat.upload) : [],
              },
            ],
          }"
          :options="{
            ...
            plugins: {
              ...
              datalabels: {
                ...
                formatter: function (value, context) {
                  return value / categories_sum > 0.05
