...
import { exchangeCodeForTokenAndLinkAccountAuthLinkAccountPost, getListOfOidcProvidersAuthProvidersGet, type OidcProvider } from '@/sdk';
...
const handleLink = async () => {
    if (form.username && form.password) {
        ...
        if (!response.error && token) {
            ...
            ElNotification.success({ title: 'Account Linked', message: 'You have successfully linked your account.' })
...
onMounted(async () => {
    if (route.query.pending)
        pending.value = route.query.pending.toString();
...
<template>
    <div style="max-width: 320px; margin: 100px auto">
        <h1>Link Account</h1>
        There is an existing account with this email address, please enter your credentials to authorize the link or
        login with an external account with which you have linked previously.
